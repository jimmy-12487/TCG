...
int main(int argc, const char* argv[]) {
	...
	tuple_agent slide(slide_args); //player
	
	
	random_placer place(place_args); //placer
...
//		std::cerr << "======== Game " << stats.step() << " ========" << std::endl;
		...
		while (true) {
			...
			if(who.name() == slide.name())	slide.put_board(&game.state());
			std::cout << who.name() + "\n";
//			std::cerr << game.state() << "#" << game.step() << " " << who.name() << ": " << move << std::endl;
		...
		if(win.name() == slide.name()) slide.last_update();
